...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/ae_config.yaml", help="Path to config yaml")
    args = parser.parse_args()
    
    # Use relative path to config if 'configs/ae_config.yaml' is used
    config_path = args.config
    if config_path == "configs/ae_config.yaml" and not os.path.exists(config_path):
        # Try to find it relative to the script's directory
        script_dir = os.path.dirname(os.path.realpath(__file__))
        rel_config_path = os.path.join(script_dir, '..', '..', 'configs', 'ae_config.yaml')
